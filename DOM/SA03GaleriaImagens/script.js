...
function atualizarCarrossel(){
    ...
    imagensUrl.forEach(imagem => {
        ...
        img.src = imagem;//atribuindo po derenço da imagem -(net)/local
        img.style.width = "100%"; //garante o ajuste de 100 da imagem
        divCarrossel.appendChild(img); //img -> div
    });

    //ajuste do carrossel -> movimentação dele 
    divCarrossel.style.width = `${imagensUrl.length*100}%`//ajusta a largura
    //iniciar o carrosel
    iniciarCarrosel();
}
//funcao para imiciar o carrosel
function iniciarCarrosel(){
    let index = 0;

    setInterval(()=>{
        index = (index+1)%imagensUrl.length;
        divCarrossel.style.transition = `transform ${(1+imagensUrl.length)/imagensUrl.length}s ease-in-out`;//tra
